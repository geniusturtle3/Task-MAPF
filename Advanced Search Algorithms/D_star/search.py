...
class DStar:
    ...
    def process_state(self):
        ...
        if kold < x.h:
            for neighbor in neighbors:
                if neighbor.h <= kold and x.h > neighbor.h + self.cost(neighbor, x):
                    x.parent = neighbor
                    x.h = neighbor.h + self.cost(neighbor, x)  
        ...
        # Else node k is smaller than h (RAISE)
            ...
            for neighbor in neighbors:
                if neighbor.tag == "NEW" or (neighbor.parent == x and neighbor.h != x.h + self.cost(neighbor, x)):
                    neighbor.parent = x
                    self.insert(neighbor, x.h + self.cost(neighbor, x))
                else:
                    if neighbor.parent != x and neighbor.h > x.h + self.cost(neighbor, x):
                        self.insert(x, x.h)
                    else:
                        if neighbor.parent != x and x.h > neighbor.h + self.cost(x, neighbor) and neighbor.tag == "CLOSED" and neighbor.h > kold:
    ...
    def repair_replan(self, node):
        ...
        while True:
            ...
            if k_min >= node.h or k_min == -1:
                break
        P = self.get_backpointer_list(node)
        return P
        #### TODO END ####
    ...
    def modify_cost(self, obstacle_node, neighbor):
        ...
        costToObstacle = self.cost(neighbor, obstacle_node)
        obstacle_node.h = min(obstacle_node.h, costToObstacle)
        # Put the obstacle node and the neighbor node back to Open list 
        if obstacle_node not in self.open:
    ...
    def prepare_repair(self, node):
        ...
        for neighbor in self.get_neighbors(node):
            ...
            if neighbor.is_dy_obs and not neighbor.is_obs:
                ...
                for nNeighbor in self.get_neighbors(neighbor):
                    self.modify_cost(neighbor, nNeighbor)
        #### TODO END ####
